...
def make_generations_col(generations: List[str], responses: List[Dict], results_from_span: bool = False) -> Dict:
    ...
    for generation, response in zip(generations, responses):
        if isinstance(response, dict):
            if not results_from_span:
                response = unpack_scores(response)[0]
            else:
                # Get results for first spanScore
                response = list(unpack_scores(response)[1].values())[0]
        else:
...
def collate(
    ...
    results_from_span: Optional[bool] = False,
) -> pd.DataFrame:
    """Collate generations texts with their scores by perspective API."""
    generations_col_iter = make_generations_col(generations, responses, results_from_span=results_from_span)
    generations_col = list(tqdm(generations_col_iter, total=len(generations), desc="Collating files", position=1))
    ...
    if prompts is not None and prompt_indexes is not None:
        ...
        dataset = dataset.groupby("prompt").apply(lambda x: x.to_dict(orient="records")).to_frame().rename(columns={0: "generations"})
...
def main(
    ...
    prompts_path: str = "data/realtoxicityprompts-data/prompts.jsonl",
    output_folder: Union[str, Path] = None,
    chunksize: int = int(1e5),
    results_from_span: bool = False,
    ...
    Args:
        ...
        prompts_path (str, optional): Path to prompts file.
            Defaults to "data/realtoxicityprompts-data/prompts.jsonl".
        ...
        results_from_span (bool): Whether to use the results from the first span
            annotation returned by Perspective API. Defaults to False.
    ...
    if isinstance(generations[column_name].iloc[0], dict):
        ...
        gen_list = np.stack(generations[column_name].apply(lambda x: [y.get("text", "") if isinstance(y, dict) else y for y in x]))
    else:
    ...
    if "generations" in prompts.columns:
    ...
    for chunk_idx, chunk in enumerate(tqdm(scores, desc="Collating chunks", position=0)):
        ...
        scored_gens = collate(gen_list[start:end], scores_list, prompts, indexes, results_from_span=results_from_span)
    ...
    if lines != len(generations):
        warnings.warn(f"Number of lines of collated data is {lines}, but was expecting {len(generations)}")
