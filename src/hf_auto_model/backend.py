...
class AutoModelBackend:
    ...
    def generate_batch(self, prompts: List[str], max_new_tokens: int = 50, **generation_kwargs) -> List[GenerationResult]:
        ...
        try:
            ...
            # Decode each generated sequence
            decoded_outputs = [
                [self.tokenizer.decode(output[prompt_token_length:], skip_special_tokens=True) for output in outputs_i]
                for outputs_i in outputs
            ]
            return [
                GenerationResult(prompt=prompt, outputs=outputs_i)
                for prompt, outputs_i in zip(prompts, decoded_outputs)
            ]
