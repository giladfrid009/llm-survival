...
def compute_toxicity_unprompted(df: pd.DataFrame, output_file: Path, threshold: float = 0.5) -> pd.DataFrame:
...
def compute_toxicity_prompted(df: pd.DataFrame, output_file: Path, threshold: float = 0.5) -> pd.DataFrame:
    ...
    for label, prompt_condition in tqdm(prompt_conditions, leave=False, desc="Prompted Toxicity Metrics"):
